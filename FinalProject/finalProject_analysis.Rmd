...
articlesAsWords <- wiredArticlesDf %>% 
  unnest_tokens(word,body,token="words", strip_punct=TRUE)

corp <- Corpus(VectorSource(wiredArticlesDf$body)) 
# %>%  tm_map(removePunctuation) %>%
#   tm_map(removeNumbers) %>%
#   tm_map(removeWords, stopwords(kind="en"))

text_dtm <- DocumentTermMatrix(corp,control = list(tolower=TRUE,
...
yo <- LDA(text_dtm3,k = 2, control = list(seed = 1234))
```
